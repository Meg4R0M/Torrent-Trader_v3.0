...
$('#membercp_form').submit(function(e){
	...
	switch(wDo){
		case'personal_details':var gender=$('#memberinfo_gender_female').is(':checked')?'Female':($('#memberinfo_gender_male').is(':checked')?'Male':''),$date_of_birth=$('input[name="date_of_birth"]');
		buildQuery+='&gender='+gender;
		if($date_of_birth.length&&$date_of_birth.val()!=''){
			buildQuery+='&date_of_birth='+$date_of_birth.val();
		}
		break;
		case'contact_details':buildQuery=$.TSUE.buildLinkQuery(buildQuery,['your_email','membercp_your_existing_password']);
		useAlert=false;
		break;
		case'preferences':var themeid=$('select[name="themeid"]').val(),languageid=$('select[name="languageid"]').val(),timezone=$('select[name="timezone"]').val(),torrentStyle=$('select[name="torrentStyle"]').val(),cids=[];
		...
		break;
		case'privacy':var visible=$('input[name="visible"]').is(':checked'),receive_admin_email=$('input[name="receive_admin_email"]').is(':checked'),receive_pm_email=$('input[name="receive_pm_email"]').is(':checked'),show_your_age=$('input[name="show_your_age"]').is(':checked'),allow_view_profile=$('select[name="allow_view_profile"]').val();
		...
		break;
		case'password':buildQuery=$.TSUE.buildLinkQuery(buildQuery,['membercp_your_existing_password','membercp_new_password','membercp_confirm_new_password']);
		break;
		case'signature':var signature=tinyMCE.activeEditor.getContent();
		buildQuery+='&signature='+$.TSUE.urlEncode(signature);
		break;
		case'invite':var invite_friend_name=$('input[name="invite_friend_name"]').val(),invite_friend_email=$('input[name="invite_friend_email"]').val(),invite_friend_message=$('textarea[name="invite_friend_message"]').val();
		...
		break;
		case'performance':var shoutbox_enabled=$('input[name="shoutbox_enabled"]').is(':checked'),irtm_enabled=$('input[name="irtm_enabled"]').is(':checked'),alerts_enabled=$('input[name="alerts_enabled"]').is(':checked');
	...
	$.ajax({
		url:TSUESettings['website_url']+'/ajax/membercp.php',data:buildQuery,success:function(serverResponse){
