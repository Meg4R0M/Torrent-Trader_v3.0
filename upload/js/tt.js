...
var buildQuery='',uAgent=navigator.userAgent.toLowerCase(),$currentRecentTorrentListPN=1,docTitleInterval=false,resizeCount=0;$.TSUE={init:function()
...
{var $BOX=$('#boxContainer'),$searchForm=$('#globalSearchForm'),$searchType,$searchKeywords;function createSubmitForm(query,fields)
...
{$('<div id="forumCategoriesCheckboxes">'+$.TSUE.clearresponse(serverResponse)+'</div>').appendTo($searchForm);}});break;case 2:$.ajax
...
{e.preventDefault();removeSearchError();$searchKeywords=$('input[name="searchKeywords"]',$searchForm).val();$searchType=parseInt($('input[name="searchType"]:checked',$searchForm).val());if(!$searchKeywords||!$searchType)
...
return false;});},getItemOffsetRight:function($whatever)
{return($(window).width()-($whatever.offset().left+$whatever.outerWidth()));},initDropDownMenu:function($deon)
{if($deon&&$deon==true)
{$('.dropdown dt a').off();$(document).bind('click');}
$('.dropdown dt a').on('click',function(e)
{e.preventDefault();var $this=$(this),$item=$this.parents('.dropdown').find('ul');if($item.is(':visible'))
{$item.hide();return false;}
$('.dropdown dd ul').hide();if($item.offset().left==0)
{$item.css({position:'absolute',left:'auto',right:0}).show();}
else
{$item.css({position:'absolute',left:0,right:'auto'}).show();}
return false;});$(document).bind('click',function(e)
...
$.TSUE.autoScroller(workWith,hash);return false;});$('[rel=selectLanguage]').on('hover',function(e)
{var $this=$(this);var $aText=$('.atext',$this);if(TSUESettings['languageCache']&&!$aText.html().match(/fb_ajax-loader/gi))
{return false;}
$.ajax
({data:'action=select_language&securitytoken='+TSUESettings['stKey'],success:function(serverResponse)
{$aText.html($.TSUE.clearresponse(serverResponse));TSUESettings['languageCache']=true;$('[rel=language]').click(function(e)
...
{return false;}
if($(window).width()<800)
{$.TSUE.jumpInternal('?p=profile&pid=18&memberid='+memberID);return false;}
if($.TSUE.memberInfoCache[memberID])
{$.TSUE.memberCard($.TSUE.memberInfoCache[memberID]);$.TSUE._timeAgo();$.TSUE.initDropDownMenu(true);return;}
$.ajax
...
{var $wW=$(window).width();$('#scrolling').remove();$('#menu').removeClass('fixedMenu');if($wW>=1000)$('.icons').removeClass('fixedICons');if($(window).scrollTop()>1)
{$('<div id="scrolling" style="position: fixed; right: 15px; bottom: 35px; padding: 0; cursor: pointer; z-index: 1000; display: none;"><img src="'+TSUESettings['theme_dir']+'buttons/up.png" alt="" title="" /></div>').prependTo('body').click(function(e)
{e.preventDefault();$('html, body').animate({scrollTop:0},'slow',TSUESettings['toggleEffect']);return false;}).fadeIn('slow');$('#menu').addClass('fixedMenu');if($wW>=1000)$('.icons').addClass('fixedICons');}});},resizeImages:function()
{if(!TSUESettings['autoResizeImages'])
{return;}
$('#content img').not('[rel=resized_by_tsue]').not('#toggle').each(function()
{var $image=$(this),$width=$image.width(),$height=$image.height(),$ratio=0,$newHeight=null,$resizedHTML;if($width>=TSUESettings['website_resize_images_max_width'])
{$resizedHTML=$('<div style="position: relative; width: 100%; display: block;" id="resizedHTML'+resizeCount+'">'+(TSUESettings['isMobile']?'':'<div style="position: absolute; top: 0; left: 0; padding: 0; text-align: center; width: 100%; padding: 15px 0; background: #000; opacity: 0.5; color: #fff;" class="pampacim'+resizeCount+'"></div>')+'</div>').insertBefore($image).click(function()
{$.fancybox({'href':$image.attr('src'),'type':'image','openEffect':'elastic','closeEffect':'elastic','maxWidth':'90%','maxHeight':'90%'});return false;});$image.appendTo($resizedHTML).attr('rel','resized_by_tsue').attr('class','pampam').css({'cursor':'pointer','max-width':'100%'});var $resizedimage=$(this);if(!TSUESettings['isMobile'])
{$('.pampacim'+resizeCount).html(TSUEPhrases['javascript_resized'].replace(/%1/i,parseInt($width)).replace(/%2/i,parseInt($height)).replace(/%3/i,$resizedimage.width()).replace(/%4/i,$resizedimage.height())).width($resizedimage.width());}
$(window).resize(function()
{var $rC=0;$('.pampam').each(function()
{var $image=$(this);$('#resizedHTML'+$rC+' div').width($image.width());$rC++;});});resizeCount++;}
else
...
{if(!$.TSUE.inOverlay())
{return false;}
$.TSUE.findOverlayTextDiv().find('#inOverlayPreview').remove();$('<div class="'+(skipClass?'':'previewBox')+'" id="inOverlayPreview">'+message+'</div>').prependTo($.TSUE.findOverlayTextDiv());},responsive:function()
{$('#responsiveMenu').slimmenu
({resizeWidth:'900',collapserTitle:'Menu',animSpeed:'medium',easingEffect:null,indentChildren:false,childrenIndenter:'&nbsp;'});},inOverlay:function()
...
{var left=($(window).width()/2)-($.TSUE.findOverlayDiv().outerWidth()/2);$.TSUE.findOverlayDiv().css('left',left);}},resizeMemberCard:function()
{if($('#showMemberCard').length)
{var left=($(window).width()/2)-($('#showMemberCard').outerWidth()/2);$('#showMemberCard').css('left',left);}},toggleTinyMCEinOverlay:function()
{$inline_editor=$('body').find('textarea[id="inline_editor"]');if($inline_editor.length)
{$.TSUE.etoggle($inline_editor,1,function()
{tinyMCE.execCommand('mceAddControl',false,'inline_editor');tinymce.execCommand('mceFocus',false,'inline_editor');});}},isTinyMCERunning:function()
{if($('body').find('textarea[id="inline_editor"]').length&&typeof(tinyMCE)=='object'&&typeof(tinyMCE.execCommand)=='function')
{return true;}
else
{return false;}},removeTinyMCEinOverlay:function()
{if($.TSUE.isTinyMCERunning())
{$inline_editor=$('body').find('textarea[id="inline_editor"]');tinyMCE.execCommand('mceFocus',false,'inline_editor');tinyMCE.execCommand('mceRemoveControl',false,'inline_editor');$inline_editor.html('').remove();}},closedialog:function()
...
({mask:{color:TSUESettings['maskColor'],loadSpeed:TSUESettings['maskSpeed'],opacity:TSUESettings['maskOpacity']},oneInstance:false,top:$topPos,fixed:$fixedPos,left:$leftPos,speed:$speed,onLoad:function()
...
$.TSUE._timeAgo();$.TSUE.resizeImages();$.TSUE.initializeFancyBOX();$.TSUE.initializeTooltip();$.TSUE.initDropDownMenu(true);$.TSUE.initAutoDescription();$.TSUE.initFilterSearch();},clearresponse:function(message)
...
{$.TSUE.init();$('#breadcrumbs').rcrumbs();});$(window).load(function()
{$.TSUE.resizeImages();$.TSUE.initFacebook();$.TSUE.autoScrollTo();});$(window).resize(function()
{$.TSUE.resizeOverlay();$.TSUE.resizeMemberCard();});$(function()
{var $menu=$('nav ul');$('#pull').on('click',function(e)
{e.preventDefault();$menu.slideToggle();return false;});$(window).resize(function()
