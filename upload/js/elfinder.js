 ...
 * Version 2.1 (Nightly: 98d2faf) (2013-01-31)
 * http://elfinder.org
 * 
 ...
 * @type String
 **/
elFinder.prototype.version = '2.1 (Nightly: 98d2faf)';
...
$.fn.elfinderbutton = function(cmd) {
	return this.each(function() {
		...
		var c        = 'class',
			...
			menu,
			button   = $(this).addClass('ui-state-default button ttip')
			...
			var button = $('<button type="button">'+name+'</button>')
				...
				.keydown(function(e) { 
					...
					if (e.keyCode == $.ui.keyCode.ENTER) {
						...
						next.length ? next.focus() : $(this).parent().children('button:first').focus()
					}
				})
...
$.fn.elfindersearchbutton = function(cmd) {
	return this.each(function() {
		var result = false,
			button = $(this).hide().addClass('ui-widget-content '+cmd.fm.res('class', 'searchbtn')+''),
			search = function() {
...
$.fn.elfindersortbutton = function(cmd) {
	...
	return this.each(function() {
		var fm       = cmd.fm,
			...
			asc      = selected+'-asc',
			desc     = selected+'-desc',
			button   = $(this).addClass('ui-state-default button elfinder-menubutton elfinder-button-'+name)
				.attr('title', cmd.title)
...
$.fn.elfindertoolbar = function(fm, opts) {
	this.not('.elfinder-toolbar').each(function() {
		var commands = fm._commands,
			self     = $(this).addClass('ui-helper-clearfix elfinder-toolbar grad3'),
			panels   = opts || [],
			l        = panels.length,
			i, cmd, panel, button;
...
elFinder.prototype.commands.download = function() {
	...
	this.getstate = function() {
		...
		return  !this._disabled && cnt && cnt == 1 && cnt == filter(sel).length ? 0 : -1;
	}
	...
	this.exec = function(hashes) {
		...
		$(iframes)
			...
			.ready(function() {
				...
				}, 20000 ); // give mozilla 20 sec + 10 sec for each file to be saved
			});
		...
		var fm    = this.fm,
			...
			open = function(file, id) {
				var dialog   = $('<div class="elfinder-dialog-resize"/>'),
					...
					uibuttonset = '<div class="ui-widget-content ui-corner-all elfinder-buttonset btn-group resize-btn-group"/>',
					uibutton    = '<div class="ui-state-default button"/>',
					...
					uitype   = $('<div class="elfinder-resize-type btn-group"/>')
						.append('<input type="radio" name="type" id="type-resize" value="resize" checked="checked" /><label for="type-resize" class="button">'+fm.i18n('resize')+'</label>')
						.append('<input type="radio" name="type" id="type-crop"   value="crop"/><label for="type-crop" class="button">'+fm.i18n('crop')+'</label>')
						.append('<input type="radio" name="type" id="type-rotate" value="rotate"/><label for="type-rotate" class="button">'+fm.i18n('rotate')+'</label>'),
					type     = $('input', uitype)
						.change(function() {
				...
				uirotate.append($(row)
					...
					.append($('<div style="float:left; width: 138px;">')
...
elFinder.prototype.commands.upload = function() {
	...
	this.exec = function(data) {
		...
		button = $('<div class="ui-state-default button ui-button-text-only"><span class="ui-button-text">'+fm.i18n('selectForUpload')+'</span></div>')
