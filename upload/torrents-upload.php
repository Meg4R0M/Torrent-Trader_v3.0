...
//
//  TorrentTrader v3.x
//      $LastChangedDate: 2016-10-05 15:53:36 +0100 (Wed, 5 Oct 2016) $
...
*/
    ...
    if ($anonyupload == "yes" || $CURUSER["privacy"] == "strong") {
        $uploader = "anonymous";
    } else {
        $uploader = "[url=".$site_config['SITEURL']."/account-details.php?id=".$CURUSER['id']."]" .$CURUSER['username']. "[/url]";
    }
    $msg_shout = sqlesc("Nouveau Torrent: [url=".$site_config['SITEURL']."/torrents-details.php?id=".$id."]".$torrent."[/url] uploaded by ".($anon == 'no' ? "par [url=".$site_config['SITEURL']."/account-details.php?id=".$CURUSER['id']."]" .$CURUSER['username']. "[/url]" : "")."");
    SQL_Query_exec("INSERT INTO shoutbox (uid, date, name, text) VALUES(0,NOW(), 'System' ,".$msg_shout.")");
