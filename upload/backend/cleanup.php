...
//END//
	...
	///annonce sur shout
	$shout = SQL_Query_exec("SELECT `id`, `descr`, `time`, `last_time` FROM `shoutannounce` WHERE `active` = 'yes'");
	while ($row = mysqli_fetch_array($shout))
	{
		if ($row['last_time'] + $row['time'] > gmtime())
			continue;

		SQL_Query_exec("INSERT INTO shoutbox (uid, date, name, text) VALUES(0,NOW(), 'System', " . sqlesc($row['descr']) . ")");
		SQL_Query_exec("UPDATE `shoutannounce` SET `last_time` = '" . gmtime() . "' WHERE `id` = " . $row['id'] . "");
	}
	///fin annonce shout
