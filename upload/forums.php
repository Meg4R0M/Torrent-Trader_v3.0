...
function update_topic_last_post($topicid) {
    ...
    $arr = mysql_fetch_row($res) or showerror(T_("FORUM_ERROR"), "No post found");
    $postid = $arr[0];
...
if ($action == "post") {
	...
	if ($newtopic) { //Create topic
		...
		$topicid = mysql_insert_id() or showerror(T_("FORUM_ERROR"),"No topic ID returned");

	}else{
		//Make sure topic exists and is unlocked
		$res = SQL_Query_exec("SELECT * FROM forum_topics WHERE id=$topicid");
		$arr = mysql_fetch_assoc($res) or showerror(T_("FORUM_ERROR"),"Topic id n/a");
		if ($arr["locked"] == 'yes')
    ...
    $postid = mysql_insert_id() or showerror(T_("FORUM_ERROR"),"Post id n/a");
...
//Get topic posts
	...
	forumheader("<a href='../community/?action=viewforum&amp;forumid=$forumid'>$forum</a> <b style='font-size:16px; vertical-align:middle'>/</b> $subject");
...
//Post Bottom
	...
	print("<tr class='p-foot'><td width='150' align='center'><a href='../user/?id=$posterid'><button type='button' class='btn btn-primary'/>Profile</button></a> <a href='../message/?compose&amp;id=$posterid'><button type='button' class='btn btn-success'/>PM</button></a></td><td>");

	print ("<div style='float: left;'><a href='../report/?forumid=$topicid&amp;forumpost=$postid'><button type='button' class='btn btn-warning'/>Report?</button></a>&nbsp;<a href='javascript:scroll(0,0);'><button type='button' class='btn btn-inverse'/>Go To Top</button></a></div><div align='right'>");
	
	//define buttons and who can use them
	if ($CURUSER["id"] == $posterid || $CURUSER["edit_forum"] == "yes" || $CURUSER["delete_forum"] == "yes"){
		print ("<a href='../community/?action=editpost&amp;postid=$postid'><button type='button' class='btn btn-info'/>Edit</button></a>&nbsp;");
	}
	if ($CURUSER["delete_forum"] == "yes"){
		print ("<a href='../community/?action=deletepost&amp;postid=$postid&amp;sure=0'><button type='button' class='btn btn-danger'/>Delete</button></a>&nbsp;");
	}
	if (!$locked && $maypost) {
...
if ($action == "deletepost") {
    ...
    $arr = mysql_fetch_row($res) or showerror(T_("ERROR"), T_("FORUMS_NOT_FOUND_POST"));
    $topicid = $arr[0];
...
if ($action == "viewforum") {
	...
	forumheader("<a href='../community/?action=viewforum&amp;forumid=$forumid'>$forumname</a>");
	
	if ($CURUSER)
		print ("<table cellpadding='0' cellspacing='5' width='100%'><tr><td><div align='right'><a href='../community/?action=newtopic&amp;forumid=$forumid'><img src='../images/button_new_post.png' alt='' /></a></div></td></tr></table>");
	...
	print("<tr class='f-title'><th align='left' colspan='2' width='100%'>Topic</th><th>Replies</th><th>Views</th><th>Author</th><th align='right'>Last post</th>\n");
      ...
      while ($topicarr = mysql_fetch_assoc($topicsres)) {
			...
			if ($tpages > 1) {
			  ...
			  for ($i = 1; $i <= $tpages; ++$i)
				$topicpages .= " <a href='../community/?action=viewtopic&amp;topicid=$topicid&amp;page=$i'>$i</a>";
			  $topicpages .= ")";
        }
        else
          $topicpages = "";
        ...
        if (mysql_num_rows($res) == 1) {
          $arr = mysql_fetch_assoc($res);
          $lpauthor = "<a href='../user/?id=$topic_userid'>$arr[username]</a>";
        }
        else
          $lpauthor = "Deluser";
        }
        ...
        $subject = ($sticky ? "<b>".T_("FORUMS_STICKY").": </b>" : "") . "<a href='../community/?action=viewtopic&amp;topicid=$topicid'><b>" .
        encodehtml(stripslashes($topicarr["subject"])) . "</b></a>$topicpages";
        print("<tr class='f-row'><td class='f-img' valign='middle'><img src='../images/$topicpic.png' alt='' />" .
	     ...
	     if ($CURUSER["edit_forum"] == "yes" || $CURUSER["delete_forum"] == "yes") {
			...
			if ($locked)
				print("<a href='../community/?action=unlocktopic&amp;forumid=$forumid&amp;topicid=$topicid&amp;page=$page' title='Unlock'><img src='../images/topic_unlock.png' alt='UnLock Topic' /></a>\n");
			else
				print("<a href='../community/?action=locktopic&amp;forumid=$forumid&amp;topicid=$topicid&amp;page=$page' title='Lock'><img src='../images/topic_lock.png' alt='Lock Topic' /></a>\n");
				print("<a href='../community/?action=deletetopic&amp;topicid=$topicid&amp;sure=0' title='Delete'><img src='../images/topic_delete.png' alt='Delete Topic' /></a>\n");
			if ($sticky)
			   print("<a href='../community/?action=unsetsticky&amp;forumid=$forumid&amp;topicid=$topicid&amp;page=$page' title='UnStick'><img src='../images/folder_sticky_new.png' alt='Unstick Topic' /></a>\n");
			else
			   print("<a href='../community/?action=setsticky&amp;forumid=$forumid&amp;topicid=$topicid&amp;page=$page' title='Stick'><img src='../images/folder_sticky.png' alt='Stick Topic' /></a>\n");
			  print("</span></td>\n");
        }
        print("</tr>\n");
      } // while
   ...
   print("</table></div>");
    ...
    if ($maypost)
		print("<td><a href='../community/?action=newtopic&amp;forumid=$forumid'><img src='../images/button_new_post.png' alt='' /></a></td>\n");
    print("</tr></table>\n");
    insert_quick_jump_menu($forumid);
    end_frame();
    stdfoot();
...
while ($forums_arr = mysql_fetch_assoc($forums_res)){
    ...
    print("<tr class='f-row'><td class='f-img'><img src='../images/$img.png' alt='' /></td><td align='left' width='100%' class='alt1'><a href='../community/?action=viewforum&amp;forumid=$forumid'><b>$forumname</b></a><br />\n" .
