...
<tr>
    ...
    <tr>
    <td align="center"><a href="admincp.php?action=shoutannounce"><img src="images/admin/blocked.png" border="0" width="32" height="32"><br />Annonce shoutbox<br /></a></td>
        <td align="center"><a href="admincp.php?action=clearshoutbox"><img src="images/admin/oust.png" border="0" width="32" height="32"><br /><?php echo T_("CLEAR_SHOUTBOX"); ?></a><br /></td>
        <td align="center"><a href="admincp.php?action=antibruteforce"><img src="images/admin/ab.png" border="0" width="32" height="32"><br />AntiBruteForce Manager<br /></a></td>
    </tr>
</table>
...
if ($action=="blocks" && $do=="view") {
    ...
    if ($action=="categories" && $do=="view"){
        stdhead(T_("".T_("CATEGORIES_MANAGEMENT").""));
        navmenu();

        begin_frame(T_("TORRENT_CATEGORIES"));
        echo "<br /><center><a href='admincp.php?action=categories&amp;do=add'><b><input type='submit' value='".T_("ADD_NEW_CATEGORY")."' /></b></a></center><br />";

        print("<center><i>".T_("PLEASE_NOTE")."</i></center><br /><br />");

        echo("<center><table cellpadding='3' width='60%' class='table_table'>");
        echo("<tr><th width='10' class='table_head'>".T_("SORT")."</th><th width='' class='table_head'>".T_("PARANT_CAT")."</th><th class='table_head'>".T_("PARANT_IMAGE")."</th><th class='table_head'>".T_("SUB_CAT")."</th><th class='table_head'>".T_("SUB_IMAGE")."</th><th width='' class='table_head'>".T_("EDIT")."</th><th width='' class='table_head'>".T_("_DEL_")."</th></tr>");
        $query = "SELECT * FROM categories ORDER BY /*parent_cat ASC,*/ sort_index ASC";/* "parent_cat ASC" is sorting by "Parant Cat Name" */  /* "sort_index ASC" is sorting by "Sort" (number) */
        ...
        while ($row = mysqli_fetch_assoc($sql)) {
            ...
            if (isset($row["image_sub"]) && $row["image_sub"] != "")
                $image_sub = "<img border=\"0\" src=\"" . $site_config['SITEURL'] . "/images/categories/" . $row["image_sub"] . "\" alt=\"" . $row["name"] . "\" />";
            else
                $image_sub = "-";

            if (isset($row["image"]) && $row["image"] != "")
                $image = "<img border=\"0\" src=\"" . $site_config['SITEURL'] . "/images/categories/" . $row["image"] . "\" alt=\"" . $row["name"] . "\" />";
            else
                $image = "-";

            print("<tr><td class='table_col1' align='center'>$priority</td><td class='table_col2'>$parent</td><td align='center' class='table_col1'>$image</td><td class='table_col2'>$name</td><td align='center' class='table_col1'>$image_sub</td>");

            print("<td class='table_col2' align='center' width='40'><a href='admincp.php?action=categories&amp;do=edit&amp;id=$id'><img src=".$site_config['SITEURL']."/images/admin/edit.png alt='[".T_("EDIT")."]' title='".T_("EDIT")."'></a></td><td align='center' class='table_col1' width='40'><a href='admincp.php?action=categories&amp;do=delete&amp;id=$id'><img src=".$site_config['SITEURL']."/images/admin/delete.png alt='[".T_("_DEL_")."]' title='".T_("_DEL_")."'></a></td></tr>");
        }
    ...
    if ($action=="categories" && $do=="edit"){
        stdhead(T_("CATEGORIES_MANAGEMENT"));
        navmenu();
        ...
        if (!is_valid_id($id))
            ...
            autolink("admincp.php?action=categories&do=view", "".T_("INVALID_ID")."");

        $res = SQL_Query_exec("SELECT * FROM categories WHERE id=$id");

        if (mysql_num_rows($res) != 1)
            //show_error_msg(T_("ERROR"), "".T_("NO_CATEGORY")." $id.",1);
            autolink("admincp.php?action=categories&do=view", "".T_("NO_CATEGORY")."");
        ...
        if ($_GET["save"] == '1'){
            ...
            if ($parent_cat == "")
                //show_error_msg(T_("ERROR"), "".T_("PARANT_CAT_CAN_NOT_BE_EMPTY")."",1);
                autolink("admincp.php?action=categories&do=view", "".T_("PARANT_CAT_CAN_NOT_BE_EMPTY")."");

            $name = $_POST['name'];
            if ($name == "")
                //show_error_msg(T_("ERROR"), "".T_("SUB_CAT_CANNOT_BE_EMPTY")."",1);
                autolink("admincp.php?action=categories&do=view", "".T_("SUB_CAT_CANNOT_BE_EMPTY")."");

            $sort_index = $_POST['sort_index'];
            $image = $_POST['image'];
            $image_sub = $_POST['image_sub'];
            $parent_cat = sqlesc($parent_cat);
            $name = sqlesc($name);
            $sort_index = sqlesc($sort_index);
            $image = sqlesc($image);
            $image_sub = sqlesc($image_sub);


            SQL_Query_exec("UPDATE categories SET parent_cat=$parent_cat, name=$name, image=$image, sort_index=$sort_index, image_sub=$image_sub WHERE id=$id");

            //show_error_msg(T_("COMPLETED"),"category was edited successfully.",1);

            autolink("admincp.php?action=categories&do=view", "".T_("CATEGORY_SUCCESSFULLY").".");

        } else {
            begin_frame(T_("CP_CATEGORY_EDIT"));
            print("<form method='post' action='?action=categories&amp;do=edit&amp;id=$id&amp;save=1'>\n");
            print("<br /><center><table border='0' cellspacing='0' cellpadding='5'>\n");
            print("<tr><td class='table_col1' align='right'><b>".T_("PARANT_CATEGORY").": </b></td><td class='table_col1' align='left'><input type='text' name='parent_cat' value=\"".$arr['parent_cat']."\" /><br /> ".T_("ALL_SUBCATS")."</td></tr>\n");
            print("<tr><td class='table_col1' align='right'><b>".T_("PARANT_IMAGE").": </b></td><td class='table_col1' align='left'><input type='text' name='image' value=\"".$arr['image']."\" /> ".T_("SINGLE_FILENAME")."</td></tr>\n");
            print("<tr><td class='table_col1' align='right'><b>".T_("SUB_CATEGORY").": </b></td><td class='table_col1' align='left'><input type='text' name='name' value=\"".$arr['name']."\" /></td></tr>\n");
            print("<tr><td class='table_col1' align='right'><b>".T_("SUB_IMAGE").": </b></td><td class='table_col1' align='left'><input type='text' name='image_sub' value=\"".$arr['image_sub']."\" /> ".T_("SINGLE_FILENAME")."</td></tr>\n");
            print("<tr><td class='table_col1' align='right'><b>".T_("SORT").": </b></td><td class='table_col1' align='left'><input type='text' name='sort_index' value=\"".$arr['sort_index']."\" /></td></tr>\n");
            print("<tr><td class='table_col1' colspan='2' align='center'><input type='submit' value='".T_("SUBMIT")."' /></td></tr>\n");
    ...
    if ($action=="categories" && $do=="delete"){
        ...
        if ($_GET["sure"] == '1'){
            ...
            autolink("admincp.php?action=categories&do=view", "".T_("CATEGORY_DELETED")."");

        }else{
            begin_frame(T_("CATEGORY_DEL"));
            print("<form method='post' action='?action=categories&amp;do=delete&amp;id=$id&amp;sure=1'>\n");
            print("<br /><center><table border='0' cellspacing='0' cellpadding='5'>\n");
            print("<tr><td class='table_col1' align='left'><b>".T_("CATEGORY_MOVE").": </b></td><td class='table_col1'><input type='text' name='newcat' /> ".T_("CAT_ID")."</td></tr>\n");
            print("<tr><td class='table_col1' colspan='2' align='center'><input type='submit' value='".T_("SUBMIT")."' /></td></tr>\n");
            print("</table></center><br />\n");
            print("</form>\n");
        }
    ...
    if ($action=="categories" && $do=="takeadd"){
        ...
        if ($name == "")
            //show_error_msg(T_("ERROR"), "".T_("SUB_CAT_CANNOT_BE_EMPTY")."",1);
            autolink("admincp.php?action=categories&do=categories&do=add", "".T_("SUB_CAT_CANNOT_BE_EMPTY")."");

        $parent_cat = $_POST['parent_cat'];
        if ($parent_cat == "")
            //show_error_msg(T_("ERROR"), "".T_("PARANT_CAT_CAN_NOT_BE_EMPTY")."",1);
            autolink("admincp.php?action=categories&do=categories&do=add", "".T_("PARANT_CAT_CAN_NOT_BE_EMPTY")."");
        ...
        SQL_Query_exec("INSERT INTO categories (name, parent_cat, sort_index, image, image_sub) VALUES ($name, $parent_cat, $sort_index, $image, $image_sub)");

        if (mysqli_affected_rows() == 1)
            //show_error_msg(T_("COMPLETED"),"Category was added successfully.",1);
            autolink("admincp.php?action=categories&do=view", "".T_("CATEGORY_WAS_ADDED_SUCCESSFULLY")."");
        else
            //show_error_msg(T_("ERROR"),"Unable to add category",1);
            autolink("admincp.php?action=categories&do=view", "".T_("ERROR")." ".T_("UNABLE_TO_ADD_CATEGORY")."");
    }
    ...
    if ($action=="categories" && $do=="add"){
        ...
        print("<input type='hidden' name='do' value='takeadd' />\n");
        print("<br /><table border='1' cellspacing='0' cellpadding='5'>\n");
        print("<tr><td class='table_col1' align='right'><b>".T_("PARANT_CATEGORY").":</b></td><td class='table_col1'><input type='text' name='parent_cat' /></td></tr>\n");
        print("<tr><td class='table_col1' align='right'><b>".T_("PARANT_IMAGE").":</b></td><td class='table_col1'><input type='text' name='image' /></td></tr>\n");
        print("<tr><td class='table_col1' align='right'><b>".T_("SUB_CATEGORY").":</b></td><td class='table_col1'><input type='text' name='name' /></td></tr>\n");
        print("<tr><td class='table_col1' align='right'><b>".T_("SUB_IMAGE").":</b></td><td class='table_col1'><input type='text' name='image_sub' /></td></tr>\n");
        print("<tr><td class='table_col1' align='right'><b>".T_("SORT").":</b></td><td class='table_col1'><input type='text' name='sort_index' /></td></tr>\n");


        print("<tr><td class='table_col1' align='center' colspan='2'><input type='submit' value='".T_("SUBMIT")."' /></td></tr>\n");
...
// Annonce Shout
if ($action == "shoutannounce"){

    if (@count($_POST['id'])) {
        foreach ($_POST['id'] as $id) {
            SQL_Query_exec("DELETE FROM `shoutannounce` WHERE `id` = $id");
        }
        retour("admincp.php?action=shoutannounce", "Annonce retirée. ",2);
    }

    stdhead("Annonce Shoutbox");
    navmenu();
    begin_frame("Annonce Shoutbox");

    echo '<div align="center">[<a href="admincp.php?action=shoutannouncecreate">Créer une nouvelle annonce</a>]</div><br />';

    $sql = SQL_Query_exec("SELECT * FROM `shoutannounce` ORDER BY `id` DESC");

    if (mysqli_num_rows($sql) > 0){
        echo("<form method='post' action='admincp.php?action=shoutannounce'>");
        echo("<table border='0' cellpadding='5' cellspacing='3' align='center' width='100%' class='ttable_headinner'>");
        echo("<tr>");
        echo("<td class='ttable_head'><strong>Titre / Éditer</strong></td>");
        echo("<td class='ttable_head'><strong>Message</strong></td>");
        echo("<td class='ttable_head'><strong>Ajouté le</strong></td>");
        echo("<td class='ttable_head'><strong>Interval</strong></td>");
        echo("<td class='ttable_head'><strong>Dernier affichage</strong></td>");
        echo("<td class='ttable_head'><strong>Active</strong></td>");
        echo("<td class='ttable_head'><strong>Effacer</strong></td>");
        echo("</tr>");

        while ($row = mysqli_fetch_assoc($sql)){
            echo("<tr>");
            echo("<td class='ttable_col1' align='center'><a href='admincp.php?action=shouteditannounce&id=" . $row['id'] . "'>" . htmlspecialchars($row['title']) . "</a></td>");
            echo("<td class='ttable_col2' align='center'>" . format_comment($row['descr']) . "</td>");
            echo("<td class='ttable_col1' align='center'>" . utc_to_tz($row['added']) . "</td>");
            echo("<td class='ttable_col1' align='center'>" . $row['time'] . " Seconds</td>");
            echo("<td class='ttable_col2' align='center'>" . utc_to_tz(date('Y-m-d H:i:s', $row['last_time'])) . "</td>");
            echo("<td class='ttable_col1' align='center'>" . oui_non($row['active']) . "</td>");
            echo("<td class='ttable_col2' align='center'><input type='checkbox' name='id[]' value='".$row['id']."'/></td>");
            echo("</tr>");
        }

        echo("</table>");
        echo("<div align='right'><input type='submit' value='Supprimer'/></div>");
        echo("</form>");
    }else{
        echo("<center>Aucune annonce actuellement.</center>");
    }
...
if ($action == "shoutannouncecreate"){

    if ($_SERVER['REQUEST_METHOD'] == 'POST'){

        if (empty($_POST['title']) || empty($_POST['descr']) || !is_valid_id($_POST['time']))
            show_error_msg("Erreur", "Données incomplètes. <a href='admincp.php?action=shoutannouncecreate'>Retour</a>?", 1);

        SQL_Query_exec("INSERT INTO `shoutannounce` (`title`, `descr`, `added`, `time`) VALUES (" . sqlesc($_POST['title']) . ", " . sqlesc($_POST['descr']) . ", " . sqlesc(get_date_time()) . ", " . sqlesc($_POST['time']) . ")");

        retour("admincp.php?action=shoutannounce", "Annonce créée. ",2);
    }

    stdhead("Créer une annonce");
    navmenu();
    begin_frame("Créer une annonce");

    echo("<form name='shoutannouncecreate' method='post' action='admincp.php?action=shoutannouncecreate'>");
    ...
    echo("</tr>");
    echo("<tr>");
    echo("<td><strong>Interval:</strong></td>");
    echo("<td><select name='time'><option value='3600'>1 Heure</option>
      <option value='7200'>2 Heures</option><option value='10800'>3 Heures</option>
      <option value='14400'>4 Heures</option><option value='18000'>5 Heures</option>
      <option value='43200'>12 Heures</option><option value='86400'>24 Heures</option>
      </select></td>");
    echo("</tr>");
    echo("</table>");
    ...
    require_once("backend/bbcode.php");
    $descr = "";
    echo(textbbcode("shoutannouncecreate","descr","$descr"));
    echo("<div align='center'><input type='reset' value='Reset' /><input type='submit' value='Envoyer' /></div>");
    echo("</form>");
...
if ($action == 'shouteditannounce'){
    $id = isset($_GET['id']) ? $_GET['id'] : '';

    if (!is_valid_id($id))
        show_error_msg("Erreur", "ID invalide", 1);

    $sql = SQL_Query_exec("SELECT * FROM `shoutannounce` WHERE `id` = $id");
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if (empty($_POST['title']) || empty($_POST['descr']) || !is_valid_id($_POST['time']))
            retour("admincp.php?action=shouteditannounce&id=$id", "Données incomplètes.",2);

        SQL_Query_exec("UPDATE `shoutannounce` SET `title` = " . sqlesc($_POST['title']) . ", `descr` = " . sqlesc($_POST['descr']) . ", `time` = " . sqlesc($_POST['time']) . ", `active` = " . sqlesc($_POST['active']) . " WHERE `id` = $id");
        retour("admincp.php?action=shoutannounce", "Annonce modifiée. ",2);
    }

    stdhead("Edition de l'annonce");
    navmenu();
    begin_frame("Edition de l'annonce");

    echo("<form name='shouteditcreate' method='post' action='admincp.php?action=shouteditannounce&id=$id'>");
    echo("<table border='0' cellpadding='5' cellspacing='0' align='center'>");
    echo("<tr>");
    echo("<td><strong>Titre:</strong></td>");
    echo("<td><input type='text' name='title' size='30' value='" . htmlspecialchars($row['title']) . "' /></td>");
    echo("</tr>");
    echo("<tr>");
    echo("<td><strong>Interval:</strong></td>");
    echo("<td><select name='time'><option value='3600' " . ($row['time'] == '3600' ? 'selected="selected"' : ''). ">1 Heure</option>
      <option value='7200' " . ($row['time'] == '7200' ? 'selected="selected"' : ''). ">2 Heures</option>
      <option value='10800' " . ($row['time'] == '10800' ? 'selected="selected"' : ''). ">3 Heures</option>
      <option value='14400' " . ($row['time'] == '14400' ? 'selected="selected"' : ''). ">4 Heures</option>
      <option value='18000' " . ($row['time'] == '18000' ? 'selected="selected"' : ''). ">5 Heures</option>
      <option value='43200' " . ($row['time'] == '43200' ? 'selected="selected"' : ''). ">12 Heures</option>
      <option value='86400' " . ($row['time'] == '86400' ? 'selected="selected"' : ''). ">24 Heures</option>
      </select></td>");
    echo("</tr>");
    echo("<tr>");
    echo("<td><strong>Active:</strong></td>");
    echo("<td><select name='active'><option value='yes' " . ($row['active'] == 'yes' ? 'selected="selected"' : '' ). ">Oui</option><option value='no' " . ($row['active'] == 'no' ? 'selected="selected"' : ''). ">Non</option></select></td>");
    echo("</tr>");
    echo("</table>");

    require_once("backend/bbcode.php");
    echo(textbbcode("shouteditcreate","descr", $row['descr']));
    echo("<div align='center'><input type='reset' value='Reset' /><input type='submit' value='Envoyer' /></div>");
    echo("</form>");
...
if ($action == "clearshoutbox") {
    ...
    begin_frame("Effacement du Contenu des Shoutboxs");

    if ($_GET['yes'] == "yes") {
        $res = SQL_Query_exec("TRUNCATE TABLE `shoutbox`");
        if ($res) echo "La shoutbox est vide!";
    } else {
        echo "Etes-vous certain de vouloir effacer le contenu des ShoutBoxs?<br>";
        echo "<a href='admincp.php?action=clearshoutbox&yes=yes'><font color=red><b><blink>Oui</b></font></blink></a>";
    }
...
/*
**************************************************************************************
*                            Anti BruteForce Manager                                 *
**************************************************************************************
*/
if ($action == "antibruteforce")
{
    global $site_config;
    $atbf_ylock = $site_config['atbf_years_lock'];
    $atbf_mlock = $site_config['atbf_months_lock'];
    $atbf_tr = $site_config['atbf_trials'];
    $atbf_dlock = $site_config['atbf_days_lock'];
    $atbf_activate = $site_config['atbf_activate'];
    if($atbf_activate === true){
        $atbf_activate = "<font color='red'>Enabled</font>";
    }else{
        $atbf_activate = "<font color='green'>Disabled</font>";
    }
    ...
    if ($do == "del")
    {
        ...
        while ($row = mysqli_fetch_row($res))
        {
            write_log("Account '$row[1]' (ID: $row[0]) was reset by $CURUSER[username]");
            resetantibruteforce($row[0]);
        }


        autolink("admincp.php?action=antibruteforce", "Entries Reset");
    }

    $count = get_row_count("users", "WHERE atbf_islocked = '1'");

    list($pagertop, $pagerbottom, $limit) = pager(25, $count, 'admincp.php?action=antibreuteforce&amp;');

    $res = SQL_Query_exec("SELECT id, username, class, email, ip, atbf_date_locked, atbf_date FROM users WHERE atbf_islocked = '1' ORDER BY username DESC $limit");


    stdhead("ANTIBRUTEFORCE");
    navmenu();

    begin_frame("ANTIBRUTEFORCE");
    ?>


    This page displays all users which are being blocking. Unlocking is done automatically on the indicated date, but you have the possibility to unlock manually.
    Please read this notes before change your config file.<br>
    <center><b><h2><u>The Good Practices</u></h2></b></center><br>
    <i><font color="red">Note: If you want to change the blocking values in your configuration file, you must reset all accounts. So:</font></i><br>
    1) Change blocking values in config file, apply.<br>
    2) Reset all accounts.<br>
    3) It's all good.<br><br>
    <b><h2>Your actual config</h2></b>
    <table border="0" cellpadding="3" cellspacing="0" width="100%" align="center" class="table_table">
        <tr>
            <th class="table_head">Is active</th>
            <th class="table_head">Nbr Trials</th>
            <th class="table_head">Years Lock</th>
            <th class="table_head">Months Lock</th>
            <th class="table_head">Days lock</th>
        </tr>
        <tr>
            <td class="table_col2" align="center"><?php echo $atbf_activate ; ?></td>
            <td class="table_col1" align="center"><?php echo $atbf_tr; ?></td>
            <td class="table_col2" align="center"><?php echo $atbf_ylock; ?></td>
            <td class="table_col1" align="center"><?php echo $atbf_mlock; ?></td>
            <td class="table_col2" align="center"><?php echo $atbf_dlock; ?></td>
    ...
    <?php if ($count > 0): ?>
    <br />
    <form id="usersearchs" method="post" action="admincp.php?action=antibruteforce">
        <input type="hidden" name="do" value="del" />
        <table border="0" cellpadding="3" cellspacing="0" width="100%" align="center" class="table_table">
            <tr>
                <th class="table_head">Username</th>
                ...
                <th class="table_head">E-mail</th>
                <th class="table_head">IP</th>
                <th class="table_head">Lock Date</th>
                <th class="table_head">Unlock Date</th>
                <th class="table_head"><input type="checkbox" name="checkall" onclick="checkAll(this.form.id);" /></th>
            </tr>
            <?php while ($row = mysqli_fetch_assoc($res)): ?>
                <tr>
                    ...
                    <td class="table_col1" align="center"><?php echo $row["email"]; ?></td>
                    <td class="table_col2" align="center"><?php echo $row["ip"]; ?></td>
                    <td class="table_col1" align="center"><?php echo $row["atbf_date_locked"]; ?></td>
                    <td class="table_col2" align="center"><?php echo $row["atbf_date"]; ?></td>
                    <td class="table_col1" align="center"><input type="checkbox" name="users[]" value="<?php echo $row["id"]; ?>" /></td>
                </tr>
            <?php endwhile; ?>
            <tr>
