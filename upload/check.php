...
//
//  TorrentTrader v3.x
//      $LastChangedDate: 2016-10-05 14:02:08 +0000 (Wed, 5 Oct 2016) $
...
<body>
	<center>
		<br /><b>TorrentTrader v3 unoffical Config Check<br />v1.7 - Meg4R0M<br /><br /></b>
		<input type="button" class="button" value="Check Again" onclick="window.location=window.location" /><br />
		<a href="check.php?phpinfo=1">PHPInfo</a>
	</center><br />
	<b>Required Settings Check:</b><br />
	<p>If any of these items are highlighted in red then please take actions to correct them. <br />Failure to do so could lead to your TorrentTrader! installation not functioning correctly.</p>
	<br />
	...
	<br />
	<p>These settings are recommended for PHP in order to ensure full compatibility with TorrentTrader!. However, TorrentTrader! will still operate if your settings do not quite match the recommended.</p>
	<table cellpadding="3" cellspacing="1" style="border-collapse: collapse" border="1">
		<tr>
	...
	?></table>
	<br />
	<b>Directory and File Permissions Check:</b><br />
	<p>In order for TorrentTrader! to function correctly it needs to be able to access or write to certain files or directories.<br />If you see "Unwriteable" you need to change the permissions on the file or directory to 777 (directories) or 666 (files) so that TorrentTrader to write to it.<br />The censor.txt should be chmodded to <b>600</b>.</p>
	<br />
	...
	if (!@($GLOBALS["___mysqli_ston"] = mysqli_connect($mysql_host,  $mysql_user,  $mysql_pass)))
		printf("<font color='#ff0000'><b>Failed to connect to database:</b></font> (%d) %s<br />", ((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_errno($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_errno()) ? $___mysqli_res : false)), ((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
	else {
		if (!@((bool)mysqli_query($GLOBALS["___mysqli_ston"], "USE " . $mysql_db)))
			printf("<font color='#ff0000'><b>Failed to select database:</b></font> (%d) %s<br />", ((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_errno($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_errno()) ? $___mysqli_res : false)), ((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
		else {
			$r = mysqli_query($GLOBALS["___mysqli_ston"], "SHOW TABLES");
			if (!$r)
				printf("<font color='#ff0000'><b>Failed to list tables:</b></font> (%d) %s<br />", ((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_errno($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_errno()) ? $___mysqli_res : false)), ((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
			else {
				$tables = array();
				...
				if (!is_numeric($site_config["default_theme"]))
					...
					$res = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT uri FROM stylesheets WHERE id=$site_config[default_theme]");
				...
				if (!is_numeric($site_config["default_language"]))
					...
					$res = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT uri FROM languages WHERE id=$site_config[default_language]");
					if ($row = mysqli_fetch_row($res)) {
